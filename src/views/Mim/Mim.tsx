...
import mim from 'src/assets/img/rpg/mim.png';
...
const Mim: React.FC = () => {
  ...
  return (
    <Page>
      <Container size="lg">
        <PlayerDiv active={isMobile}>
          <Heading>
            <ul>
              <p>Magic Faucet for those who finished the maze</p>
              <p></p>
            </ul>
          </Heading>
          <StakeButtonStyled
            onClick={() =>
              window.open(
                'https://www.cryptovoxels.com/spaces/6448ec7f-c9dc-4df4-8f1f-6d23fb4ade0a/play',
                '_blank',
              )
            }
          >
        ...
        <StakeButtonStyled1 active={isMobile} onClick={showStake}>
          Magik Faucet
        </StakeButtonStyled1>

        <Glow active={isMobile}>
          <Rpgplayer onClick={showStake} src={mim} active={isMobile} />
        </Glow>
      </Container>
    </Page>
  );
...
`;
const StakeButtonStyled1 = styled.button<{ active?: boolean }>`
  font-size: 1rem;
  font-weight: 600;
  color: none;
  backdrop-filter: blur(9px);
